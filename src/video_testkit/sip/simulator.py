...
class SimulatorDeviceState:
    ...
    status: str = "IDLE"  # IDLE / REGISTERING / REGISTERED / REGISTER_FAILED
...
class DeviceSimulator:
    """为场景中的每台设备维护 SIP 注册状态，并按需触发 REGISTER 流程。"""
    ...
    async def _register_flow(
        ...
        try:
            ...
            msg2 = build_message(
                f"REGISTER {uri} SIP/2.0",
                self._base_headers(branch2, device_id, uri, call_id, from_tag, 2, local_port)
                + [("Authorization", auth)],
            )
            transport.sendto(msg2, target)
            resp2 = await self._await_response(protocol, branch2, timeout)
            if resp2.status_code() != 200:
                raise SimulatorError(f"鉴权 REGISTER 预期 200，实际 {resp2.status_code()}")
    ...
    def _base_headers(
        ...
        return [
            ...
            ("Max-Forwards", "70"),
            ("Expires", str(self._settings.gb_expires)),
